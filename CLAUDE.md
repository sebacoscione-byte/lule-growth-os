...
- 2026-07-10: Se sumó el scope `instagram_business_manage_insights` al OAuth de Instagram (`src/app/api/instagram-business/auth/route.ts`) y `getBusinessDiscovery()` en `src/lib/instagram-business.ts`, para poder consultar datos públicos (seguidores, bio, cantidad de posts) de otras cuentas Business/Creator por username — caso de uso inicial: comparar contra la cuenta de Instagram de CIMEL Lanús. Probado contra el token ya conectado (`@draluciachahin`) antes del cambio: sin este scope, Meta devuelve `"Tried accessing nonexisting field (business_discovery)"` — confirma que era necesario. **Requiere reconectar Instagram manualmente una vez desplegado** (Meta exige reautorizar cuando cambia el scope pedido, no alcanza con el deploy) — ver pendiente en `docs/IN_PROGRESS.md`.
...
3. Scopes requeridos: `instagram_business_basic`, `instagram_business_content_publish`, `instagram_business_manage_insights`
   (este último habilita Business Discovery — consultar datos públicos de otras cuentas Business/Creator por username,
   vía `getBusinessDiscovery()` en `src/lib/instagram-business.ts`)
