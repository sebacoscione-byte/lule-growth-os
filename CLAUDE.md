...
`cron/weekly-report/route.test.ts` como referencia): se importa `GET`/`POST`/`PATCH` directo del
`route.ts` y se llama como una función común, mockeando `@/lib/supabase/server` con `jest.mock`
para no pegarle a la base real. **Requisito**: `jest.config.js` necesita `moduleNameMapper` para
el alias `@/` — sin eso, `jest.mock("@/lib/x")` no resuelve (un `import` normal sí funciona porque
Next lo reescribe en compilación, pero `jest.mock()` recibe un string literal que Jest debe
resolver por su cuenta). Ya está configurado; tenerlo en cuenta si algún día se toca ese archivo.
