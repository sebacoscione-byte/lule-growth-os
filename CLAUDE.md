...
- 2026-07-11 (TECH-01 de `docs/BACKLOG.md`): `src/middleware.ts` renombrado a `src/proxy.ts` (convención de Next.js 16 — función `middleware()` → `proxy()`, `export const config` → `export const proxyConfig`; confirmado con la skill `vercel:nextjs` que es un rename 1:1 sin cambio de comportamiento, y verificado con el dev server que `/`, `/dashboard` sin sesión y `/privacidad` redirigen/responden igual que antes). El warning de deprecación en cada arranque ya no aparece. De paso: `npm run lint` quedó en 0 problemas (se sacó un import de tipo sin usar, `ContentChannel`, residual de cuando se sacó Google Business del frente de Estudio de contenido). Se re-chequeó la vulnerabilidad moderada de PostCSS (transitiva de `next`) — sigue sin solución real, no existe ningún `16.3.0` estable todavía (solo canaries/preview), así que sigue pendiente esperar un patch estable de Next. **Queda pendiente** la parte de headers de seguridad (CSP, etc.) que también pedía este ticket — no se tocó por el riesgo real de romper un flujo de OAuth en silencio sin poder probarlo de punta a punta en este entorno (sin credenciales de login).
