...
// (publicaciones) o la bio (para pedir turno), nunca a "guardar". Segunda vuelta, mismo dia: con esa
// regla ya aplicada, Seba marco que el cierre seguia siendo "muy brusco" -- ej. "Link en la bio para
// tu turno en Lomas de Zamora." o "Charlemos en la consulta para que recuperes tu tranquilidad.":
// fragmentos telegraficos o vagos, no una invitacion calida en primera persona. Dio la correccion
// exacta: "Te invito a entrar en mi bio para conocer mas y solicitar un turno." y "Hace clic en el
// link de mi bio para agendar una consulta y recuperes tu tranquilidad." -- el cierre tiene que sonar
// a que Lucia te esta hablando a vos, no a un cartel administrativo, y tiene que nombrar
// explicitamente la accion (entrar a la bio / hacer clic en el link de la bio), no solo aludir a ella.
// Tercera vuelta, mismo dia: generando un lote de 10 historias reales de una para llenar 2 semanas de
// cola, 7 de 10 volvieron a usar "guardar" en el cierre -- pero NINGUNA repitio literal las 4 frases
// exactas ya prohibidas ("guarda esto/esta historia/esta info/esta lista"): el modelo encontro
// variantes nuevas que esquivan la lista literal sin dejar la idea de fondo ("guardá esta PLACA",
// "guardalo", "guarda esto para tu proximo chequeo preventivo" -- todas conjugaciones/objetos
// distintos, mismo patron prohibido). Ademas 2 de las 2 excepciones a esa regla siguieron literal
// ("guarda esto", "guardá esta info") pese a estar en la lista -- confirma que ademas de ampliar el
// alcance, el modelo simplemente no sigue la regla el 100% de las veces (no determinismo, ya
// documentado en otros lados de este archivo). Regla reescrita para prohibir la RAIZ "guard-" en
// cualquier conjugacion/objeto, no una lista cerrada de frases literales -- y se sumaron 2 ejemplos
// validos mas (variedad) para no empujar al modelo siempre hacia los mismos 3 ejemplos.
// Cuarta vuelta, mismo dia: Seba marco que el cierre siempre terminaba empujando a "agendar desde
// la bio" y pidio variedad -- tambien invitar a entrar a la bio solo para informarse mas, sin
// mencionar turno. Revisando el porque, se encontro la causa real de fondo de gran parte de la
// insistencia con "guardar" en la ronda anterior: OBJECTIVE_GUIDANCE (mas abajo en este archivo,
// compartida por post/historia/carrusel) le dice explicitamente al modelo, para el objetivo
// "educacion", que "el CTA invita a guardar la pieza" -- correcto para post/carrusel (ahi "guardar"
// es un habito real de Instagram), pero implicitamente en conflicto directo con la prohibicion de
// "guardar" de historia. La mayoria de las piezas del lote de reposicion de cola tenian objetivo
// "educacion", lo que probablemente reforzaba esa tension. Regla nueva: dos familias de cierre
// validas (informarse, sin turno / pedir turno, con agenda), eligiendo segun el objetivo editorial
// -- y una aclaracion explicita de que la sugerencia de "guardar" de OBJECTIVE_GUIDANCE no aplica a
// "historia", usar la familia de informarse en su lugar.
...
- PROHIBIDO el verbo "guardar" en CUALQUIER conjugacion o variante como cierre -- esto incluye, sin limitarse a: "guarda", "guardá", "guardalo", "guardala", "guardate", "guarda esto/esta/este", seguido de CUALQUIER palabra (ej. "guardá esta placa", "guardalo para tu consulta", "guardá esta info util" son TODOS invalidos igual, no solo la frase exacta "guarda esto"). No importa que tan bien continue la oracion despues: si en algun punto del cierre aparece la raiz "guard-" pedida como accion, es un error. La razon es la misma siempre: no tiene sentido pedirle a alguien que "guarde" una historia en esta cuenta (a diferencia de un post, no es un habito real de este formato aca) -- ninguna variante de esa idea es valida.
- PROHIBIDO tambien un cierre telegrafico, tipo cartel o fragmento suelto -- ej. "Link en la bio para pedir turno", "Link en mi bio para turnos", "Turnos: link en la bio", o algo vago que ni siquiera menciona la bio, como "Charlemos en la consulta". El cierre de una historia SIEMPRE es una oracion calida y COMPLETA, en primera persona, como si Lucia te estuviera invitando a vos directamente (mismo tono cercano-profesional del resto de la cuenta) -- y tiene que nombrar explicitamente la accion concreta: entrar a la bio o hacer clic en el link de la bio -- nunca solo aludir a que "hay un link" sin decir que hacer con el, y NUNCA la palabra "guardar".
- El cierre NO siempre tiene que empujar a pedir turno -- varia segun el objetivo editorial de la pieza (ver la guia de objetivo en el pedido). Dos familias validas, elegi la que corresponda:
  1) Informarse (objetivos alcance/educacion/confianza): invita a entrar a la bio o al perfil para CONOCER MAS o SEGUIR INFORMANDOSE sobre el tema, sin mencionar turno ni agenda. Ejemplos: "Entra a mi bio para conocer mas sobre esto.", "Te invito a entrar en mi bio para informarte mejor.", "Mira mi perfil, ahi tenes mas info sobre el tema."
  2) Pedir turno (objetivo conversion, o cuando el tema puntual lo amerita): invita explicitamente a agendar/reservar el turno desde la bio. Ejemplos: "Te invito a entrar en mi bio para conocer mas y pedir tu turno.", "Hace clic en el link de mi bio para agendar tu consulta y despejar esa duda.", "Agenda tu turno ingresando al link de mi bio."
  IMPORTANTE: si la guia de objetivo de mas abajo sugiere "guardar la pieza" o "invitar a guardar" como CTA (esa sugerencia esta pensada para post/carrusel, donde "guardar" es un habito real de Instagram), para HISTORIA esa sugerencia especifica NO aplica -- usa en su lugar la familia 1 (informarse) de arriba, nunca la palabra "guardar". Variá la redaccion entre piezas -- no repitas siempre la misma estructura ni caigas siempre en la familia 2 (turno) aunque sea la mas facil de escribir.
- Patron recomendado: dar UN dato o idea concreta y real en pocas palabras (nunca una promesa vacia de que hay datos en otro lado) + el cierre calido de arriba. Ejemplo real que sirve de modelo: "¿Sabias que un electrocardiograma mide esto, esto y esto? Te invito a entrar en mi bio para conocer mas" -- ahi "esto, esto y esto" representa 2-3 datos reales y cortos que SI van escritos, no una promesa vacia.
- Mucho mas corto que un post: pensalo como un cartel que se lee entero en dos segundos, no como el titular de una nota -- pero el cierre calido de arriba no es opcional aunque sume unas palabras mas.
...
- TEXT — READ CAREFULLY. The image must show exactly two lines of visible text: a large headline, and a smaller subtitle below it. Here they are, each wrapped in quotes so you can see exactly where each one starts and ends:
    Headline text (quoted, verbatim): "${input.visual_headline}"
    Subtitle text (quoted, verbatim): "${input.visual_subtitle}"
  The ONLY text allowed anywhere in the entire image is exactly those two quoted Spanish strings above, rendered ONCE each, character for character and letter for letter, keeping every accent (á é í ó ú) and the letter ñ, and keeping every word in the exact same order. Do NOT translate, rephrase, abbreviate, shorten, split, reorder or drop any word (for example, never drop an article such as "LA"). Spell every single word correctly and proof-read the spelling before finalizing (for example it must read "ALARMA", never "ALAMA"). The words "headline" and "subtitle" used above are only labels in these instructions to tell you which quoted string is which — they are English words that must NEVER themselves appear as visible text in the image.
- Do NOT render ANY other text: no third line, no byline, no credential or title, no doctor name, no caption, no field label like "headline" or "subtitle", no watermark, no logo, no English words, no descriptive words, and no invented, decorative or garbled lettering. The CREATIVE DIRECTION above is written in English and only describes the SCENE to paint — none of its words may EVER appear as visible text inside the image.
...
- CRITICAL — NEVER HYPHENATE OR SPLIT A WORD ACROSS TWO LINES: every word must stay whole, on a single line, from its first letter to its last (for example "seguros" must never be broken into "se-" on one line and "guros" on the next — that is a real defect, not acceptable print-style hyphenation). When wrapping, only break the line BETWEEN whole words, never inside one. If a single word would not fit on the current line, move that entire word down to start the next line instead of splitting it — never insert a hyphen "-" into the middle of a word for any reason.
