...
// V2.2: la fotografia sigue ocupando todo el lienzo. La cobertura marfil ahora entra desde abajo y
// atraviesa el ancho completo, como un revelado editorial sobre la misma foto, en vez de formar un
// panel lateral. La identidad se sostiene con tipografia, paleta, ritmo y firma; la escena puede
// variar sin que el feed parezca una grilla de plantillas repetidas.
const SCRIM_FADE_START_RATIO = 0.25
const SCRIM_SOLID_START_RATIO = 0.62
const SCRIM_MAX_ALPHA = 250
const PANEL_PADDING_X = 72
const PANEL_RIGHT_MARGIN = 110
 ...
 *
 * Layout V2.2: foto full-bleed + cobertura paper que aparece gradualmente desde abajo a todo el ancho.
 * El bloque editorial vive en el tercio inferior y deja la escena reconocible arriba, sin una frontera
 * vertical entre "texto" e "imagen". El sistema conserva texto determinista, contraste y zonas seguras
 * sin depender de que el modelo dibuje tipografia correctamente.
 */
export async function composeContentPlate(input: {
  ...
  const scrimFadeStart = Math.round(height * SCRIM_FADE_START_RATIO)
  const scrimSolidStart = Math.round(height * SCRIM_SOLID_START_RATIO)
  const availableWidth = width - PANEL_PADDING_X - PANEL_RIGHT_MARGIN
  ...
  // En historias/reels la interfaz de Instagram tapa parte del borde inferior. El bloque completo
  // queda por encima de esa zona; la cobertura puede continuar hasta el borde porque no comunica.
  const bottomSafeInset = input.format === "historia" || input.format === "reel" ? 250 : 62
  const startY = Math.max(
    Math.round(height * 0.43),
    Math.round(height - bottomSafeInset - totalBlockHeight)
  )
  ...
  try {
    ...
    const filters = [
      ...
      // Cobertura editorial inferior: geq modifica solo el alfa del color source. Como depende de Y
      // y no de X, la foto nunca queda dividida en dos columnas ni aparece un borde vertical.
      `[0:v]format=rgba,geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':` +
        `a='if(lte(Y,${scrimFadeStart}),0,` +
        `if(gte(Y,${scrimSolidStart}),${SCRIM_MAX_ALPHA},` +
        `${SCRIM_MAX_ALPHA}*(Y-${scrimFadeStart})/(${scrimSolidStart}-${scrimFadeStart})))'[scrim]`,
      `[photo][scrim]overlay=x=0:y=0:format=auto[withscrim]`,
      `[withscrim]drawtext=fontfile='${fraunces}':textfile='${escapeFilterPath(headlineTextPath)}':` +
