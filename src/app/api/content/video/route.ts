...
import { getVideoPromptQualityIssues } from "@/lib/video-prompt"
...
// V2 usa la respuesta sincronica de Gemini Omni; V1 conserva el proceso asincronico de Veo. 280s deja
// margen debajo del limite de Vercel (300s) para ambas rutas, la composicion con FFmpeg y el upload.
...
export async function POST(request: Request) {
  try {
    ...
    const promptIssues = getVideoPromptQualityIssues(videoPrompt, version)
    ...
    // Musica de fondo sin copyright (2026-07-28): reemplaza el audio ambiente del motor por una
    ...
    // proveedor no se conserva como URL propia, y nunca queremos que el navegador reciba el
    ...
    if (normalized.includes("quota") || normalized.includes("resource_exhausted") || normalized.includes("rate limit")) {
      return NextResponse.json({
        ...
        error: "La clave de Gemini no tiene cuota disponible para generar video. Revisá el billing en Google AI Studio y volvé a intentar.",
